function [team_num, stat_cols, OPR, DPR] = get_event_team_stats_vs_match(filename)
...
OPR = zeros(num_matches, num_teams, size(Score_For,2));
DPR = zeros(size(OPR));

for match = 1:num_matches
    % solve Team_Matrix*OPR = Score_For for OPR
    % solve Team_Matrix*DPR = Score_Against for DPR
    inv_team_matrix = pinv(Team_Matrix(1:2*match,:));
    for k=1:size(Score_For,2)
        OPR(match,:,k)  = inv_team_matrix*Score_For(1:2*match,k);
        DPR(match,:,k)  = inv_team_matrix*Score_Against(1:2*match,k);
    end
end